...
import React from "react"; // Zakładam, że masz też useState, jeśli przycisk Menu ma otwierać SideMenu
...
export default function Profile() {
  ...
  const formFields = [
    {
      id: "name",
      label: "Your name", // Etykieta widoczna nad polem
      type: "text",
      icon: <User className="h-5 w-5 text-gray-500" />,
      value: "Your name", // Tekst wyświetlany w polu (zamiast danych użytkownika)
    },
    {
      id: "email",
      label: "Your email",
                ...
                <div key={field.id} className="mb-6 last:mb-0">
                  ...
                  <div className="relative">
                    ...
                    <input
                      id={field.id}
                      type={field.type}
                      value={field.value} // Ustawiono wartość statyczną
                      readOnly // Pole jest tylko do odczytu
                      className="h-[52px] w-full bg-slate-50/70 border border-slate-300 text-slate-800 rounded-lg pl-10 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow shadow-sm" // Usunięto hover:shadow-md, bo nie ma interakcji
                    />
                  </div>
                </div>
              ))}
              {/* Przycisk "Save Changes" został usunięty */}
            </div>
          </div>
        </div>
